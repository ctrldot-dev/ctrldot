// bootstrap_productledger registers the Product Ledger policy set for ProductLedger:*.
// Run after migrations (0003_productledger_namespaces.sql). Requires DB_URL.
//
// Usage: go run ./cmd/bootstrap_productledger [path/to/productledger-policyset.yaml]
package main
...
func main() {
	...
	yamlPath := "productledger-policyset.yaml"
	if len(os.Args) > 1 {
		yamlPath = os.Args[1]
	}
	...
	ps := domain.PolicySet{
		ID:          "productledger-policyset",
		NamespaceID: "ProductLedger",
	...
	fmt.Println("Registered productledger-policyset for ProductLedger:*")
}
